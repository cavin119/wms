<template>
  <component :is="menuComponent" :routerInfo="routerInfo" v-if="!routerInfo.is_hidden">
...
export default {
  ...
  computed: {
    menuComponent() {
      if (this.routerInfo.children&&this.routerInfo.children.filter(item=>!item.is_hidden).length) {
        return 'AsyncSubmenu'
      } else {
        return 'MenuItem'
      }
